...
namespace :load_mp do
  desc "Load mp https://scraperkharkivdeputy.herokuapp.com/"
  task :all => :environment do
    data_mps = JSON.load(open("https://scraperkharkivdeputy.herokuapp.com/"))
    data_mps.each do |m|
      if m["end_date"].nil?
        end_date = "9999-12-31"
      else
  ...
  task :image => :environment do
    mps = JSON.load(open("https://scraperkharkivdeputy.herokuapp.com/"))
    mps.each do |m|
      p m["photo_url"]
